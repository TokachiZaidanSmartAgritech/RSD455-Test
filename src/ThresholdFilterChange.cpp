/* =========================================================== */
/*
 * @file ThresholdFilterChange.cpp
 * @License MIT License. See LICENSE file in directory.
 * @brief 取得した画像に深度閾値フィルターを0.15m～2.5mに設定しそれ以外をマスクした画像
 * @date 08/22/24
...
//        std::cout << "distance_center : " << distance_center << "\n";
        ...
        // 閾値フィルター
        float threshold_min = std::max(0.1f, std::min(distance_center - 0.15f, 2.5f));
        float threshold_max = std::max(threshold_min, std::min(distance_center + 0.15f, 2.5f));
